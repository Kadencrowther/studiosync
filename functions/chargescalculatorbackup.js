...
app.post("/calculateFamilyCharges", async (req, res) => {
  ...
  try {
    ...
    logAndCapture(`Request received for family: ${familyData?.id || familyData?.FamilyId} in studio: ${studioId}`);
    ...
    logAndCapture(`Calculating charges for family: ${familyData.id || familyData.FamilyId} in studio: ${studioId}`);
    
    // Calculate charges logic
    const result = await calculateFamilyCharges(familyData, studioId, logAndCapture);
    ...
    logAndCapture(`Error in calculateFamilyCharges API: ${error.message}`, error.stack);
...
async function fetchStudioData(studioId, log) {
  ...
  const studioDoc = await admin.firestore()
    .collection('Studios')
    .doc(studioId)
    .get();
...
async function fetchStudentFamilyMax(studioId, log) {
  ...
  try {
    ...
    const maxSettingsSnapshot = await admin.firestore()
      .collection('Studios')
      .doc(studioId)
    ...
    if (!maxSettingsSnapshot.empty) {
      ...
      log(`Found StudentFamilyMax settings - Student Max: $${maxSettings.StudentMax || 0}, Family Max: $${maxSettings.FamilyMax || 0}`);
      return {
...
async function fetchFamilyFees(studioId, familyId, log) {
  ...
  const familyFeesSnapshot = await admin.firestore()
    .collection('Studios')
    .doc(studioId)
    .collection('Families')
    .doc(familyId)
...
function processFamilyFees(fees, currentDate = new Date(), log) {
  ...
  fees.forEach(fee => {
    ...
    if (fee.IsRecurring) {
      ...
      const nextUnpaidPayment = fee.Schedule.find(s => 
        s.Status === 'Unpaid' && 
        new Date(s.DueDate) >= currentDate
      );

      if (nextUnpaidPayment) {
        log(`Found next unpaid recurring payment for ${fee.Name}: $${nextUnpaidPayment.Amount} due ${nextUnpaidPayment.Month}`);
        processedFees.push({
          id: fee.id,
          name: fee.Name,
      ...
      if (unpaidPayment) {
        ...
        const dueDate = unpaidPayment.DueDate ? new Date(unpaidPayment.DueDate) : null;
        const shouldChargeNow = !dueDate || dueDate <= currentDate;
        
        if (shouldChargeNow) {
          log(`Found applicable unpaid one-time fee ${fee.Name}: $${unpaidPayment.Amount}`);
          processedFees.push({
            id: fee.id,
            name: fee.Name,
          ...
          log(`Skipping future one-time fee ${fee.Name}, due ${unpaidPayment.Month}`);
        }
      }
    }
  });
...
async function calculateFamilyCharges(familyData, studioId, log = console.log) {
  try {
    log(`Starting charge calculation for family: ${familyData.id || familyData.FamilyId}`);
    ...
    if (!studentFamilyMax) {
      ...
      log(`Found StudentFamilyMax settings in studio document - Student Max: $${studentFamilyMax.StudentMax || 0}, Family Max: $${studentFamilyMax.FamilyMax || 0}`);
    }
    ...
    const familyRatePlan = fullFamilyData.RatePlan || studioData.DefaultRatePlan || 'Monthly';
    const defaultRatePlanId = Object.values(ratePlans).find(rp => rp.Name === familyRatePlan)?.id || 
                             Object.keys(ratePlans)[0];
    
    log(`Using rate plan: ${familyRatePlan}, default rate plan ID: ${defaultRatePlanId}`);
    
    // Step 2: Calculate tuition for each student
    log('Beginning student charge calculations');
    for (const student of students) {
      log(`Processing student: ${student.FirstName} ${student.LastName} (${student.id})`);
      
      // First, determine relevant rate plans for this student's classes
      const studentClasses = await fetchRelevantClassesForStudent(studioId, student, classesData, log);
      const relevantRatePlans = determineRelevantRatePlans(studentClasses, ratePlans, defaultRatePlanId, log);

      // Then calculate tuition
      const tuition = calculateRatePlanTuitionEnhanced(student, studentClasses, ratePlans, defaultRatePlanId, log);
      ...
      const studentDiscountResult = calculateStudentDiscounts(student, studentClasses, allDiscounts, familyPromoCodes);
      ...
      log(`Student processing complete. Student total: $${studentCharges[studentCharges.length-1].total.toFixed(2)}`);
    }
    ...
    const familySpecificDiscounts = allDiscounts.filter(discount => 
      discount.IsActive && 
    ...
    familySpecificDiscounts.forEach(discount => {
      ...
      if (discount.DiscountType === 'Percentage') {
        ...
        log(`Applied percentage family discount: ${discount.Name}, ${discount.Amount}% of $${totalTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
      } else { // Fixed amount
        discountAmount = parseFloat(discount.Amount) || 0;
        log(`Applied fixed family discount: ${discount.Name}, $${discountAmount.toFixed(2)}`);
      }
    ...
    if (students.length > 1 && ratePlans) {
      log(`Checking for multi-student discounts with ${students.length} students`);
      
      // Find rate plans with family discounts
      Object.values(ratePlans).forEach(ratePlan => {
        if (ratePlan.FamilyDiscount && ratePlan.FamilyDiscount.StudentThreshold && ratePlan.FamilyDiscount.Amount) {
          ...
          log(`Found family discount in rate plan ${ratePlan.Name}: ${amount}${type === 'percentage' ? '%' : '$'} for ${threshold}+ students`);
          ...
          if (students.length >= threshold) {
            ...
            if (type === 'percentage') {
              ...
              log(`Applied percentage multi-student discount: ${amount}% of $${totalTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
            } else { // Fixed amount
              discountAmount = parseFloat(amount) || 0;
              log(`Applied fixed multi-student discount: $${discountAmount.toFixed(2)}`);
            }
    ...
    if (familyPromoCodes.length > 0) {
      ...
      const codeDiscounts = allDiscounts.filter(discount => 
        discount.IsActive && 
        discount.AssociationType === 'Code' && 
        familyPromoCodes.some(code => code.toUpperCase() === discount.DiscountCode?.toUpperCase())
      );
      ...
      codeDiscounts.forEach(discount => {
        ...
        if (discount.DiscountType === 'Percentage') {
          discountAmount = (totalTuition * discount.Amount) / 100;
          log(`Applied percentage family promo code discount: ${discount.Name}, ${discount.Amount}% of $${totalTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
        } else { // Fixed amount
          discountAmount = parseFloat(discount.Amount) || 0;
          log(`Applied fixed family promo code discount: ${discount.Name}, $${discountAmount.toFixed(2)}`);
        }
    ...
    studentCharges.forEach(student => {
      if (student.discountTotal > 0) {
        ...
        log(`Added student discount: $${student.discountTotal.toFixed(2)}, total discount now: $${totalDiscount.toFixed(2)}`);
      }
    });
    ...
    processedFees.forEach(fee => {
      logAndCapture(`- ${fee.name}: $${fee.amount.toFixed(2)} (${fee.type}) due ${fee.month}`);
    });
    ...
    if (familyDiscounts && familyDiscounts.length > 0) {
      familyDiscounts.forEach(discount => {
        logAndCapture(`- ${discount.name}: -$${discount.amount.toFixed(2)} (${discount.type})`);
      });
    }
    if (studentCharges) {
      studentCharges.forEach(student => {
    ...
    console.error(`Error in calculateFamilyCharges: ${error.message}`, error.stack);
    return {
      success: false,
      error: error.message || 'An unexpected error occurred',
 ...
 */
async function fetchRelevantClassesForStudent(studioId, student, allClasses, log) {
  log(`Fetching relevant classes for student: ${student.FirstName} ${student.LastName} (${student.id})`);
  ...
  for (const classId of student.Classes) {
    ...
    if (classData) {
      log(`Found class data for ${classId}: ${classData.ClassName || 'Unnamed'}`);
 ...
 */
function determineRelevantRatePlans(studentClasses, allRatePlans, defaultRatePlanId, log) {
  ...
  for (const classData of studentClasses) {
    ...
    if (ratePlanId && allRatePlans[ratePlanId]) {
      if (!relevantRatePlans.has(ratePlanId)) {
        log(`Adding rate plan ${ratePlanId} (${allRatePlans[ratePlanId].Name || 'Unnamed'}) for class ${classData.ClassName || classData.id}`);
        ...
        existingEntry.totalHours += (classData.Duration ? classData.Duration / 60 : 1);
        log(`Added class ${classData.ClassName || classData.id} to rate plan ${ratePlanId}, total hours now: ${existingEntry.totalHours}`);
      }
    } else {
      log(`No valid rate plan found for class ${classData.ClassName || classData.id}, using default`);
      // If no valid rate plan, use default
      if (!relevantRatePlans.has(defaultRatePlanId) && allRatePlans[defaultRatePlanId]) {
  ...
  log(`Determined ${relevantRatePlans.size} relevant rate plans for these classes`);
 ...
 */
function calculateRatePlanTuitionEnhanced(student, studentClasses, allRatePlans, defaultRatePlanId, log = console.log) {
  ...
  classesForCalc.forEach(cls => {
    ...
    log(`Added class ${cls.name}: ${hoursForClass} hours to rate plan ${ratePlanId}, total now: ${classesByRatePlan[ratePlanId].hours}`);
  });
  ...
  for (const [ratePlanId, data] of Object.entries(classesByRatePlan)) {
    ...
    log(`Rate plan cost: $${cost.toFixed(2)}, total tuition now: $${(totalTuition + cost).toFixed(2)}`);
...
function calculateRatePlanTuition(student, classes, ratePlans) {
  ...
  console.log(`Student details: ${JSON.stringify(student.FirstName)} ${JSON.stringify(student.LastName)}`);
  ...
  console.log(`Classes for calculation: ${JSON.stringify(classes.map(c => ({
    id: c.id,
  ...
  classes.forEach(cls => {
    ...
    console.log(`Added class ${cls.Name || cls.ClassName || cls.id}: ${hours} hours to rate plan ${ratePlanId}, total now: ${hoursByRatePlan[ratePlanId]}`);
  });
  ...
  if (Object.keys(hoursByRatePlan).length === 0) {
    ...
    console.log(`No rate plans assigned, using direct fees: $${totalFees.toFixed(2)}`);
    return totalFees;
  }
  ...
  Object.entries(hoursByRatePlan).forEach(([ratePlanId, totalHours]) => {
    ...
    console.log(`Rate plan cost: $${cost.toFixed(2)}, total tuition now: $${totalTuition.toFixed(2)}`);
  });
...
function calculateRatePlanCost(hours, ratePlan) {
  ...
  const hasHourRates = ratePlan && ratePlan.HourRates && Array.isArray(ratePlan.HourRates) && ratePlan.HourRates.length > 0;
  const hasTiers = ratePlan && ratePlan.Tiers && Array.isArray(ratePlan.Tiers) && ratePlan.Tiers.length > 0;
  
  if (!hasHourRates && !hasTiers) {
    console.log('No rate structure found (neither HourRates nor Tiers), returning 0');
    return 0;
  }
  ...
  if (hasHourRates) {
    ...
    if (applicableRate) {
      console.log(`Found applicable rate: ${hours} hours <= ${applicableRate.Hours} hours, rate: $${applicableRate.Rate.toFixed(2)}`);
      ...
      console.log(`No matching rate, using highest rate: ${highestRate.Hours} hours, rate: $${highestRate.Rate.toFixed(2)}`);
    ...
    if (applicableTier) {
      console.log(`Found applicable tier: ${hours} hours <= ${applicableTier.Hours} hours, cost: $${applicableTier.Cost.toFixed(2)}`);
      return applicableTier.Cost;
    } else if (sortedTiers.length > 0) {
      // If no tier matches, use the highest tier
      const highestTier = sortedTiers[sortedTiers.length - 1];
      console.log(`No matching tier, using highest tier: ${highestTier.Hours} hours, cost: $${highestTier.Cost.toFixed(2)}`);
...
function getClassFees(classDoc) {
  ...
  if (classDoc.Fee && Array.isArray(classDoc.Fee) && classDoc.Fee.length > 0) {
    console.log(`Found ${classDoc.Fee.length} fees in class document: ${JSON.stringify(classDoc.Fee)}`);
    ...
    classDoc.Fee.forEach(fee => {
      ...
      console.log(`Processing class fee: Amount: $${amount.toFixed(2)}, Type: ${type}, Name: ${name}`);
      
      // All fees in the class.Fee array are considered "class fees"
      feeResult.classFeeTotal += amount;
      console.log(`Added to class fees, total now: $${feeResult.classFeeTotal.toFixed(2)}`);
    });
  } else {
...
function getRelevantFees(student, classes, allFees) {
  console.log(`Getting relevant fees for student: ${student.FirstName} ${student.LastName}`);
  ...
  const relevantFees = allFees.filter(fee => {
    ...
    switch (fee.AssociationType) {
      ...
      case 'Family':
        ...
        if (isFamilyFee) {
          console.log(`Fee ${fee.Name} is associated with this student's family`);
        }
        return isFamilyFee;
      ...
      case 'Class':
        ...
        if (isClassFee) {
          console.log(`Fee ${fee.Name} is associated with class ${fee.AssociationItemId}`);
        }
        return isClassFee;
      ...
      case 'Season':
        ...
        if (isSeasonFee) {
          console.log(`Fee ${fee.Name} is associated with season ${fee.AssociationItemId}`);
        }
        return isSeasonFee;
      ...
      default:
        console.log(`Fee ${fee.Name} has unknown association type: ${fee.AssociationType}`);
        return false;
    }
  });
  
  console.log(`Found ${relevantFees.length} relevant fees for student from Fees collection`);
  ...
  relevantFees.forEach(fee => {
    ...
    if (feeName.includes('registr') || feeName.includes('regist')) {
      ...
      console.log(`Added ${fee.Name} as registration fee: $${feeAmount.toFixed(2)}`);
...
function calculateStudentDiscounts(student, classes, allDiscounts, promoCodes = []) {
  console.log(`Calculating discounts for student: ${student.FirstName} ${student.LastName}`);
  
  if (!classes || classes.length === 0 || !allDiscounts || !Array.isArray(allDiscounts)) {
  ...
  const allPromoCodes = [...studentPromoCodes, ...familyPromoCodes, ...(promoCodes || [])];
  ...
  classes.forEach(classObj => {
    console.log(`Processing discounts for class: ${classObj.ClassName || classObj.id}`);
    ...
    classDiscounts.forEach(discount => {
      ...
      if (discount.DiscountType === 'Percentage') {
        ...
        console.log(`Applied percentage discount: ${discount.Name}, ${discount.Amount}% of $${classTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
      } else { // Fixed amount
        discountAmount = parseFloat(discount.Amount) || 0;
        console.log(`Applied fixed discount: ${discount.Name}, $${discountAmount.toFixed(2)}`);
      }
    ...
    seasonDiscounts.forEach(discount => {
      ...
      if (discount.DiscountType === 'Percentage') {
        discountAmount = (classTuition * discount.Amount) / 100;
        console.log(`Applied percentage season discount: ${discount.Name}, ${discount.Amount}% of $${classTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
      } else { // Fixed amount
        discountAmount = parseFloat(discount.Amount) || 0;
        console.log(`Applied fixed season discount: ${discount.Name}, $${discountAmount.toFixed(2)}`);
      }
  ...
  const studentDiscounts = allDiscounts.filter(discount => 
    discount.IsActive && 
  ...
  const totalTuition = classes.reduce((sum, cls) => sum + (cls.Tuition || 0), 0);
  ...
  studentDiscounts.forEach(discount => {
    ...
    if (discount.DiscountType === 'Percentage') {
      discountAmount = (totalTuition * discount.Amount) / 100;
      console.log(`Applied percentage student discount: ${discount.Name}, ${discount.Amount}% of $${totalTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
    } else { // Fixed amount
      discountAmount = parseFloat(discount.Amount) || 0;
      console.log(`Applied fixed student discount: ${discount.Name}, $${discountAmount.toFixed(2)}`);
    }
  ...
  if (allPromoCodes.length > 0) {
    const codeDiscounts = allDiscounts.filter(discount => 
      discount.IsActive && 
      discount.AssociationType === 'Code' && 
      allPromoCodes.some(code => code.toUpperCase() === discount.DiscountCode?.toUpperCase())
    );
    ...
    codeDiscounts.forEach(discount => {
      ...
      if (discount.DiscountType === 'Percentage') {
        discountAmount = (totalTuition * discount.Amount) / 100;
        console.log(`Applied percentage promo code discount: ${discount.Name}, ${discount.Amount}% of $${totalTuition.toFixed(2)} = $${discountAmount.toFixed(2)}`);
      } else { // Fixed amount
        discountAmount = parseFloat(discount.Amount) || 0;
        console.log(`Applied fixed promo code discount: ${discount.Name}, $${discountAmount.toFixed(2)}`);
      }
...
function applyStudentFamilyMaxLimits(studentRatePlanTotals, familyRatePlanTotal, studentFamilyMax) {
  ...
  if (studentMax > 0) {
    ...
    Object.entries(studentRatePlanTotals).forEach(([studentId, tuition]) => {
      if (tuition > studentMax) {
        console.log(`StudentFamilyMax check: Student ${studentId} tuition ($${tuition.toFixed(2)}) exceeds max ($${studentMax}), capping at max`);
        adjustedStudentTotals[studentId] = studentMax;
      } else {
        console.log(`StudentFamilyMax check: Student ${studentId} tuition ($${tuition.toFixed(2)}) is under max ($${studentMax})`);
        adjustedStudentTotals[studentId] = tuition;
      }
    });
    
    // Calculate adjusted family total after student caps
    adjustedFamilyRatePlanTotal = Object.values(adjustedStudentTotals).reduce((sum, amount) => sum + amount, 0);
    console.log(`StudentFamilyMax check: After student caps, family total is $${adjustedFamilyRatePlanTotal.toFixed(2)}`);
  } else {
  ...
  if (familyMax > 0 && adjustedFamilyRatePlanTotal > familyMax) {
    console.log(`StudentFamilyMax check: Family total rate plan tuition ($${adjustedFamilyRatePlanTotal.toFixed(2)}) exceeds max ($${familyMax}), capping at max`);
    finalFamilyRatePlanTotal = familyMax;
  } else if (familyMax > 0) {
    console.log(`StudentFamilyMax check: Family total rate plan tuition ($${adjustedFamilyRatePlanTotal.toFixed(2)}) is under max ($${familyMax})`);
  }
  
  // Calculate total reduction from the caps
  const totalReduction = familyRatePlanTotal - finalFamilyRatePlanTotal;
  console.log(`StudentFamilyMax check: Total tuition reduction from caps: $${totalReduction.toFixed(2)}`);
